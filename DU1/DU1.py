...
for i in clear_words:
    if i not in word_occurence:
        ...
        word_occurence[i] = word_occurence[i] + 1
print(word_occurence)

highest_occurence_numbers = sorted(word_occurence.values(), reverse=True)[:3]

result = list()

for i in word_occurence:
    if word_occurence[i] in highest_occurence_numbers:
        print(word_occurence[i])
        result.append((word_occurence[i], i))
        print(result)
print(result)
